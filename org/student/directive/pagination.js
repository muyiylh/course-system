...
student.directive('sPagination',[function(){//自定义指令
    return {
        restrict: 'E',//仅限元素名调用
        ...
        '<li  ng-show="conf.totalItems > 0 && showPageFlag">'+
        '当前 <input type="text" ng-model="jumpPageNum" maxlength="6" id="gotoval" style="width:20px; height:16px;margin-top:-3px;padding-top:2px;padding-left:10px;"  ng-keyup="jumpToPage($event)"/>页 ' +
        '每页<select ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions "></select>' +
        '共{{ conf.totalItems }}条' +
        '</li>' +
        '</ul>' +
        '<div class="no-items" ng-show="conf.totalItems <= 0">数据加载中</div>' +
        '</div>',
        replace: true,
        scope: {
            conf: '='//双向绑定数据
