...
var crossTable = {
    table: function(args){
        ...
        var html = "<table class = 'table_wrapper table' id='tableWrapper'><thead>",
            colContent = "<tr>";
        for (var i = 0; i < chartConfig.groups.length; i++) {
            ...
            for (var c = 0; c < colList.length; c++) {
                ...
                " class=" + colList[c].property + "><div class='th-inner'>" + colList[c].data + "</div></th>" :
                "<th class=" + colList[c].property + "><div class='th-inner'>" + colList[c].data + "</div></th>";
            }
            colContent += "</tr><tr>";
        }
        for (var k = 0; k < data[chartConfig.groups.length].length; k++) {
            colContent += "<th class=" + data[chartConfig.groups.length][k].property + "><div class='th-inner'>" + data[chartConfig.groups.length][k].data + "</div></th>";
        }
        ...
        $(container).addClass("fixed-table-container");
        $(container).html("<div class='exportBnt'><button class='btn btn-success'>导出</button></div><div class='fixed-table-body' style='width: 99%;max-height:" + tall + "px;overflow: auto'>" + html + "</div>");
        this.export();
    },
    export: function() {
        var idTmr;
